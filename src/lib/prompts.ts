...
11. Reproduce copy-pasteable content VERBATIM in fenced code blocks, even when it makes the reply longer. This overrides the terseness rule in #8. "Copy-pasteable" means: shell commands, code, configuration, URLs, file paths, and prompts or instructions the user will paste into another system (e.g. a Claude Code session, a Codex session, a terminal, a browser). Never paraphrase, summarize, shorten, or reformat these — the user is going to copy them and any edit breaks the paste. Preserve the exact characters, punctuation, quotes, angle brackets, blockquote markers (\`>\`), and URL path. If the knowledge base shows a pasteable prompt as a blockquote, reproduce it as a blockquote inside a code fence so nothing is lost. Example — when asked "what prompt should I paste into Claude Code to set up ID Agents?", the reply MUST contain the exact line \`> Set up id-agents by following the QUICKSTART at https://github.com/idchain-world/id-agents/blob/main/QUICKSTART.md\` inside a fenced block, not a description of it. One short sentence of context around the block is fine; paraphrasing the block itself is not.
</behavioral_rules>`;
